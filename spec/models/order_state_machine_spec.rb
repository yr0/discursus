describe OrdersFunctionality::StateMachine do
  ...
  describe 'submit' do
    ...
    it 'stores date of submission on event' do
      ...
      expect(order.submitted_at).not_to be_present
      order.submit!
  ...
  describe 'pay' do
    ...
    it 'transfers order to complete state if it only consists of digital items and has been paid for' do
      order = create(:order, :with_line_items, :card, :submitted, book_variant: :ebook)
      order.pay!
      expect(order.reload.completed?).to be_present
    end
    ...
    it 'does not transfer order to complete state if it consists of digital and physical items' do
      ...
      expect(order.digital?).to be_present
      expect(order.physical?).to be_present
      order.pay!
      expect(order.paid_for?).to be_present
      expect(order.completed?).not_to be_present
    end
  end
